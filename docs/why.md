...
`go-ruby-syslog/syslog` reimplements Ruby's `syslog` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-syslog/syslog` directly, with no Ruby runtime;
